...
class pickleSession:
    def __init__(self,name,material):
        ...
        to_compress = open("pickle_jar/"+self.name+".pickle","rb")
        string = to_compress.read()
        compstring = zlib.compress(string)
        to_compress.close()
        
        to_save = open("pickle_jar/"+self.name+".zzz","wb")
        to_save.write(compstring)
        to_save.close()
        
        os.remove("pickle_jar/"+self.name+".pickle")
        
class snackTime:
    def __init__(self,name):
        self.name = name
        to_decompress = open("pickle_jar/"+self.name+".zzz","rb")
        string = to_decompress.read()
        decompstring = zlib.decompress(string)
        to_decompress.close()
        
        to_save = open("pickle_jar/"+self.name+".pickle","wb")
        to_save.write(decompstring)
        to_save.close()
        
        print("Juicing Pickle...")
        pickle_in = open("pickle_jar/"+self.name+".pickle","rb")
